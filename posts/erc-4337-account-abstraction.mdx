...
```mermaid
flowchart TD
    User["User/EOA"] -->|UserOp| Bundler["Bundler"]
    Bundler -->|handleOps| EntryPoint["EntryPoint"]
    EntryPoint --> Factory["Factory<br/>(if needed)"]
    EntryPoint --> SmartAccount["SmartAccount<br/>(validate)"]
    EntryPoint --> Paymaster["Paymaster<br/>(if used)"]
```
